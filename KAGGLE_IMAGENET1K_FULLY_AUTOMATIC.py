"""
🔥🚀 ImageNet-1K AST Training - FULLY AUTOMATIC VERSION 🚀🔥

✨ ZERO MANUAL INTERVENTION REQUIRED ✨
- Auto-saves checkpoints every epoch
- Auto-commits to Kaggle output
- Auto-resumes after session timeout
- Just click "Run" and forget!

Copy this ENTIRE script into ONE Kaggle notebook cell and run!
When session times out, just click "Run" again - it continues automatically!
"""
...
class Config:
    ...
    # Checkpoint directories - use BOTH for redundancy
    checkpoint_dir = "/kaggle/working/checkpoints"
    output_dir = "/kaggle/working"  # This gets auto-saved by Kaggle
...
def auto_save_to_output():
    ...
    Copy critical files to ensure they persist across sessions.
    """
    try:
        # Create a progress file that Kaggle will save
        ...
        if os.path.exists(config.resume_checkpoint):
            ...
            # Also create a human-readable status file
            status_file = f"{config.output_dir}/STATUS.txt"
            with open(status_file, 'w') as f:
                f.write("=" * 70 + "\n")
                f.write("IMAGENET-1K AST TRAINING STATUS\n")
                f.write("=" * 70 + "\n")
                f.write(f"Last Epoch Completed: {ckpt['epoch']}/{config.num_epochs}\n")
                f.write(f"Best Validation Accuracy: {ckpt['best_acc']:.2f}%\n")
                f.write(f"Total Training Time: {ckpt['total_time']/3600:.1f} hours\n")
                f.write(f"Epochs Remaining: {config.num_epochs - ckpt['epoch']}\n")
                f.write(f"Last Updated: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write("=" * 70 + "\n")
                f.write("\nTo resume: Just run this script again!\n")
                f.write("The script will automatically detect and resume from this checkpoint.\n")

            print(f"✅ Auto-saved progress to {progress_file}")
...
print("=" * 80)
print("🔥🚀 IMAGENET-1K AST TRAINING (FULLY AUTOMATIC) 🚀🔥")
print("=" * 80)
...
if resume_from_checkpoint:
    ...
    print("📥 Auto-resuming from last saved state...\n")
    ...
    if os.path.exists(progress_file):
        ...
        print(f"   Previous session ended at Epoch {progress['last_epoch']}")
        print(f"   Best accuracy so far: {progress['best_accuracy']:.2f}%")
        print(f"   Total time invested: {progress['total_time_hours']:.1f} hours")
        print(f"   Resuming from Epoch {progress['last_epoch'] + 1}...\n")
else:
...
print("=" * 80)
print(f"💾 Auto-save enabled - checkpoints saved every epoch to /kaggle/working/")
print(f"🔄 Session will auto-resume if interrupted - just re-run this script!")
print("=" * 80)
...
for epoch in range(start_epoch, config.num_epochs + 1):
    ...
    for batch_idx, (images, labels) in enumerate(train_loader):
        ...
        if (batch_idx + 1) % 100 == 0:  # Less frequent printing to reduce clutter
    ...
    if epoch % 2 == 0 or epoch == start_epoch:
        ...
        axes[1, 1].axis('off')
        sessions_completed = int(cumulative_time / 3600 / 9) + 1
        summary_text = f"""
╔══════════════════════════════════════════════════════╗
...
║                                                      ║
║  Session #{sessions_completed}         Time: {cumulative_time/3600:.1f}h / ~100h total  ║
║  Est. Remaining:     {(cumulative_time/epoch)*(config.num_epochs-epoch)/3600:.1f} hours                 ║
║                                                      ║
║  💾 Auto-saving every epoch ✅                       ║
║  🔄 Auto-resume enabled ✅                           ║
║                                                      ║
║  Status: {'✅ ON TRACK!' if val_acc >= 30 and info['savings'] >= 70 else '⏳ Training...'}                                 ║
╚══════════════════════════════════════════════════════╝

  When session times out → Just re-run script!
        """
        axes[1, 1].text(0.5, 0.5, summary_text, fontsize=10, family='monospace',
                       ha='center', va='center',
                       bbox=dict(boxstyle='round,pad=1', facecolor='lightyellow',
                                edgecolor='orange', linewidth=3))

        plt.tight_layout()
        plt.savefig(f'{config.output_dir}/training_progress_latest.png', dpi=150, bbox_inches='tight')
        plt.savefig(f'{config.checkpoint_dir}/training_progress_epoch{epoch}.png', dpi=150, bbox_inches='tight')
        plt.show()
    ...
    # AUTOMATIC CHECKPOINT SAVE EVERY EPOCH
    checkpoint_state = {
        'epoch': epoch,
    ...
    # Save to /kaggle/working/ which Kaggle auto-saves
    torch.save(checkpoint_state, config.resume_checkpoint)

    # Auto-save progress files
    auto_save_to_output()

    # Also save numbered checkpoint every 5 epochs for safety
    if epoch % 5 == 0:
